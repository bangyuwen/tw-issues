...
Allowed dispositions are `CURRENT`, `UPDATE_REQUIRED`, `OPEN_WITH_CUTOFF`, `MOVE_OUT_OF_OPEN_QUESTIONS`, and `BLOCKED`. `OPEN_WITH_CUTOFF` uses an exact bounded text tuple: finding `No identifiable public result was located within the recorded search scope by the retrieval cutoff.`, proof scope `This evidence records only a bounded public search completed by the retrieval cutoff.`, and limitations `The search may be incomplete; later, unpublished, or unindexed results may exist.` Fixing all three fields at what the recorded search established prevents free text from asserting that no result exists. The deterministic reducer maps all current or valid moved propositions to `READY`; adds bounded open gaps as `READY_WITH_OPEN_GAPS`; and maps any required update, unsupported evidence, unbounded wording, or blocked proposition to `BLOCKED_STALE_DATA`.
