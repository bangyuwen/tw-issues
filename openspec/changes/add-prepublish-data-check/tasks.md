...
- [ ] 1.2 Confirm the receipt is caller-supplied and excluded from Git, the public bundle, and static output.

## 2. Implement the deterministic check

- [ ] 2.1 Add the dependency-free Node validator for base-to-HEAD scope derivation, exact receipt bindings, evidence boundaries, dispositions, and outcomes.
...
- [ ] 3.1 Add focused tests for one-topic factual scope, presentation-only `NOT_APPLICABLE`, newer official results, attributed-source non-promotion, and honest `OPEN_WITH_CUTOFF` gaps.
- [ ] 3.2 Add focused tests for omitted or duplicate scope, revision/data drift, invalid outcomes, private material, no network calls, and no worktree mutation.
