...
- [ ] 2.1 Add the dependency-free Node validator that reads base/HEAD Git blobs, rejects public-path worktree drift, and validates a closed receipt schema, exact bindings, evidence boundaries, dispositions, and outcomes.
...
- [ ] 3.1 Add focused tests for one-topic factual scope, date-only `as_of`/`lastUpdated` scope and mirror mismatch, presentation-only `NOT_APPLICABLE`, newer official results, attributed-source non-promotion, and honest `OPEN_WITH_CUTOFF` gaps.
