...
- Keep the longer page-range guide, checked excerpt, and document-content/TW Issues-analysis separation in `#primary-document-reading`, immediately after the gateway and before the reading legend and directory, without duplicating provenance content or repeating a standalone non-conclusion checklist.
...
- Make the Hsinchu table of contents describe only the dossier that follows the complete primary-document reading: Chapter 01 starts at `#context`, followed by `#responsibility-lines` and `#coverage-limits`; it does not repeat either primary-document destination above it.
