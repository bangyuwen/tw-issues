...
function SpeakerGroups({ groups, sourceLinks }: { groups: DossierPageModel["attributedSpeakerGroups"]; sourceLinks: (ids: string[]) => ReactNode }) {
  ...
  return <div className="speaker-groups">{groups.map((group, groupIndex) => {
    const direct = group.claims.slice(0, 2);
    const remainder = group.claims.slice(2);
    const groupOffset = offsets[groupIndex];
    const statements = (claims: typeof group.claims, start: number, zone: "direct" | "remainder") => <div className="speaker-statement-list">{claims.map((claim, index) => <div data-claim-zone={zone} key={`${group.speaker.name}-${start + index}`}><EventDisclosure className="speaker-statement-row"><summary><span className="speaker-statement-ordinal">{String(start + index + 1).padStart(2, "0")}</span><span className="speaker-statement-title">{claim.statement}</span><span className="event-disclosure-action" aria-hidden="true">展開資料</span></summary><div className="speaker-statement-body"><dl><div><dt>這能確認</dt><dd>{claim.proofScope}</dd></div><div><dt>這不能證明</dt><dd>{claim.limitations.join("；")}</dd></div></dl><div className="citations">{sourceLinks(claim.sources.map(({ publicRef }) => publicRef))}</div></div></EventDisclosure></div>)}</div>;
    return <section className="speaker-group" key={`${group.speaker.name}-${groupIndex}`}><header><strong>{group.speaker.name}</strong><span>{group.speaker.role}</span></header>{statements(direct, groupOffset, "direct")}{remainder.length > 0 && <details className="claim-remainder"><summary>展開其餘 {remainder.length} 項說法</summary>{statements(remainder, groupOffset + 2, "remainder")}</details>}</section>;
  })}</div>;
}
