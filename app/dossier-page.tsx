...
function SpeakerGroups({ groups, sourceLinks }: { groups: DossierPageModel["attributedSpeakerGroups"]; sourceLinks: (ids: string[]) => ReactNode }) {
  ...
  return <div className="speaker-groups">{groups.map((group, groupIndex) => {
    ...
    return <section className="speaker-group" key={`${group.speaker.name}-${groupIndex}`}><header><strong>{group.speaker.name}</strong><span>{group.speaker.role}</span></header><div className="speaker-group-summary"><p className="eyebrow">一句話摘要</p><p>{summary}</p><small>依公開說法整理；不代表已確認或完整。</small></div><details className="speaker-group-details"><summary><span>展開 {group.claims.length} 項具名說法</span><span className="speaker-group-details-action" aria-hidden="true">展開細節</span></summary>{statements(group.claims, groupOffset)}</details></section>;
  })}</div>;
}
