...
function PoliticalNarrativesSection({ narratives, sourceLinks }: { narratives: PoliticalNarrative[]; sourceLinks: (ids: string[]) => ReactNode }) {
  return <section className="evidence-section narrative-section" id="narratives" aria-label="政治敘事與擴散">
    ...
    <div className="narrative-matrix" role="list">{narratives.map((narrative) => <article className={`narrative-row narrative-row--${narrative.status}`} key={narrative.publicKey} role="listitem">
      ...
      <div><h3>{narrative.headline}</h3><p>{narrative.statement}</p><dl><div><dt>這能確認</dt><dd>{narrative.proofScope}</dd></div><div><dt>這不能證明</dt><dd>{narrative.limitations.join("；")}</dd></div></dl>{narrative.amplification && narrative.amplification.length > 0 && <p className="narrative-amplification"><strong>可回查擴散</strong>{narrative.amplification.map((item) => `${item.channel}（${item.publishedAt}）：${item.description}`).join("；")}</p>}<div className="citations">{sourceLinks(narrative.sources.map(({ publicRef }) => publicRef))}</div></div>
    </article>)}</div>
  </section>;
}
