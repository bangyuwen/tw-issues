...
export default function DossierPage({ model }: { model: DossierPageModel }) {
  ...
  const sourceLinks = (sourceIds: string[]) => sourceIds.map((id) => {
    ...
    return <a className="citation" data-source-ref={isCaseDossier ? source.publicRef : undefined} href={isCaseDossier ? source.canonicalUrl : `#${source.publicRef}`} target={isCaseDossier ? "_blank" : undefined} rel={isCaseDossier ? "noreferrer" : undefined} key={source.publicRef} aria-label={`來源 ${number}：${source.publisher}｜${source.title}（${source.publishedAt}${isCaseDossier ? "，另開新分頁" : ""}）`}><span aria-hidden="true">{source.publisher}</span><span className="citation-tooltip" role="tooltip"><span>{source.publisher} · {source.publishedAt}</span><strong>{source.title}</strong><small>{isCaseDossier ? "點擊開啟完整來源（另開新分頁）" : "點擊跳至完整來源"}</small></span></a>;
  });
